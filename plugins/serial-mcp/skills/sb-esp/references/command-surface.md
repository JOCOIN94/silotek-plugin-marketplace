...
| 의도 | 명령 | risk |
|---|---|---|
| WiFi country/TX power 상태 | `STWIFI` | R0 ⚠ 빌드 260505 에선 채널 변경 — critical confusions 선확인 |
...
- **빌드가 atlas 기준과 다르면 R0 등급을 신뢰하지 마라.** 같은 이름의 명령이 구버전 빌드에서 다른 동작일 수 있다 — 보드당 세션 첫 명령 전에 `FWVER`(R0) 1회로 빌드를 확정한다. 확인된 skew: **빌드 260505 의 `STWIFI` 는 조회가 아니라 WiFi 채널 변경(R2급)** → 260505 에선 `STWIFI` 금지, 채널 확인은 `CHANNEL`(R0)로 대체하고 country/TX power 확인이 필요하면 멈추고 사람에게 확인한다. 목록에 없는 빌드·skew 를 발견하면 실행을 멈추고 atlas 보강 대상으로 보고한다.
...
- **부팅 window(리셋~boot 완료) 중 송신 금지** — setup 창은 단일 키를 boot-menu 로 먹는다(`D`=리플래시, R3 급). boot 완료 시그니처 확인 전엔 R0 조회 포함 어떤 명령도 보내지 않는다(절차는 ops.md '함정·해석').
- STM32로 명령을 전달하는 R1(`COIN`/`OPERFUNCT`/`MGWANGCHA`)은 하위 STM 동작까지 검증 대상이다.

## 진단 — 차감/카운트다운이 시작되지 않을 때

펌웨어 동작(실측): 차감·카운트다운은 **카드 인식이 유지된 상태에서만** 시작된다. "카드를 댄 채 버튼"과 "버튼 먼저, 곧 카드"는 둘 다 동작하지만, **태그해서 잔액 조회가 성공했어도 카드를 뗀 뒤 버튼을 누르면 무효**다 — 이때 STM이 입력을 조용히 무시하고 **로그가 한 줄도 남지 않는다**. 즉 무로그 = 트랜잭션 미시작이지, 통신·하드웨어 고장의 증거가 아니다.

1. **재현 순서부터 확인한다** — "잔액 조회는 되는데 차감이 안 됨"의 전형은 조회 후 카드를 뗀 순서다(사용자는 '카드 인식 됐다'고 확신하는 상태라 순서를 의심하지 않는다). 카드를 **올려둔 채** 버튼을 누르는 재현으로 대조한다.
2. **트랜잭션 시작 증거**는 SB-STM 로그의 `Remaing TotalAmount` 갱신 줄이다 — 있으면 시작, 없으면 미시작.
3. 카드 유지 상태에서도 무로그면 그때 ESP↔STM 통신·하드웨어를 의심한다. 무로그 상태에서 같은 명령 반복 재시도는 금지(ops.md '무로그도 단서다').
