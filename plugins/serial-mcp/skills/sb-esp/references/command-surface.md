...
## 진단 — "차감이 안 된다" (코인/기능 무반응)

1순위 원인은 **카드 부재**다: 코인/기능 차감은 카드가 리더에 올려진 상태가 전제이며, 카드 부재 시 버튼·명령이 무시되고 **로그도 남지 않는다**(무로그가 정상 동작 — 고장 신호가 아님).

1. 사람에게 **카드를 올린 상태**로 동작 재현을 요청한다.
2. 그 상태에서 로그를 재관찰한다(차감 이벤트·STM 전달 로그).
3. 카드를 올렸는데도 무로그면 그때 통신(ESP↔STM)·하드웨어를 의심한다 — 무로그 상태에서 같은 명령 반복 재시도 금지(ops.md '무로그도 단서다').
