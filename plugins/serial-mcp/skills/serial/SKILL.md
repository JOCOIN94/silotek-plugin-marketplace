...
- SSM 실장비 검증에서는 명령 목록 smoke test로 `send_serial_command(command="HELP", port="SSM", wait_ms=1000)`를 승인받아 호출하고, `Command : RESET, REFLASHESP, ...` 형태의 응답 `lines`를 확인한다. 실장비 확인 결과 `/help`와 `/HELP`는 명령 목록을 출력하지 않고 에코만 됐다. 이 예시는 SSM 펌웨어용 smoke test이며 다른 보드에 일반화하지 않는다.
