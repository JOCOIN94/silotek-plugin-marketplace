...
## 복구 — 하위장비가 그룹에 안 잡힘 (GID/채널 불일치)

증상: 하위장비(SB 등)는 정상 부팅·로그를 내는데 SSM `INFO` 테이블에 없거나 ComRate 0. 1순위 원인: 하위장비 펌웨어 롤백/교체로 GID·채널이 SSM 과 틀어짐.

1. 양쪽에서 `GID`·`CHANNEL`(R0) 값을 대조한다(하위장비 측 명령은 해당 board 스킬).
2. 불일치면 **하위장비 쪽을 SSM 값에 맞춘다** — 하위장비 board 스킬의 `SETCONFIG`(R2, interactive)로 GID·채널만 수정(blank=현재값 유지).
3. 하위장비 재부팅 후 SSM `INFO` 재조회 — 해당 유닛 등장 + ComRate 상승으로 합류를 검증한다.

함정: 현장에 SSM 이 2대 이상이면 **어느 SSM 이 기준인지 먼저 확정**한다(포트↔보드 매핑은 ops.md '보드 식별') — 테스트베드 SSM 값에 맞추면 운영망에서 이탈한다.
