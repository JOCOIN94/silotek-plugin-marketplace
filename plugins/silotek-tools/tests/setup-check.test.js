...
test('checkLatestVersion reports ok when remote matches current version', async () => {
  const fetcher = async () => ({
    ok: true,
    status: 200,
    json: async () => ({ plugins: [{ name: 'silotek-tools', version: CURRENT_VERSION }] })
  });
...
test('checkLatestVersion warns when remote advertises a newer version', async () => {
  const fetcher = async () => ({
    ok: true,
    status: 200,
    json: async () => ({ plugins: [{ name: 'silotek-tools', version: '99.0.0' }] })
  });
  const result = await checkLatestVersion(PLUGIN_ROOT, { fetcher, skipEnv: '' });
  assert.equal(result.status, 'warn');
...
test('checkLatestVersion honors SILOTEK_TOOLS_SKIP_UPDATE_CHECK=1', async () => {
  let called = false;
