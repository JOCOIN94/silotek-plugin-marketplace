 ...
 * npm 의 `version` 라이프사이클 훅에서 호출된다 (cwd = plugins/silotek-tools).
...
const entry = entries.find(item => item && item.name === 'silotek-tools');
if (!entry) {
  console.error('sync-version: marketplace.json 에서 silotek-tools 항목을 찾지 못함');
  process.exit(1);
}
