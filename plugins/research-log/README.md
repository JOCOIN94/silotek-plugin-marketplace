...
`research-log`는 Silotek 연구 로그와 편집용 다이어그램을 위한 Claude Code 플러그인 패키지입니다.
...
1. `/research-log:research-log-yaml-create`는 소스 모드(`conversation`/`folder`/`mixed`)와 연구 성격(`구축`/`분석`/`검증`)을 결정하며, 모호한 경우 사용자에게 확인을 받습니다.
...
7. `/research-log:research-log-docx-create`가 저장된 YAML로부터 DOCX를 빌드합니다.
...
node .\plugins\research-log\scripts\setup-check.js
node .\plugins\research-log\scripts\list-yaml.js
node .\plugins\research-log\scripts\next-basename.js --title "<연구 주제>" --date <YYYY-MM-DD> --json
node .\plugins\research-log\scripts\save-draft.js "<중앙 inputs/...yaml>" --mode folder
node .\plugins\research-log\scripts\resolve-yaml.js 1 --json
...
```bash
npm install --prefix ./plugins/research-log
node ./plugins/research-log/scripts/setup-check.js
node ./plugins/research-log/scripts/list-yaml.js
node ./plugins/research-log/scripts/next-basename.js --title "<연구 주제>" --date <YYYY-MM-DD> --json
node ./plugins/research-log/scripts/save-draft.js "<중앙 inputs/...yaml>" --mode folder
node ./plugins/research-log/scripts/resolve-yaml.js 1 --json
node ./plugins/research-log/scripts/build-docx.js 1
node ./plugins/research-log/scripts/next-diagram-path.js --standalone --json
npm test --prefix ./plugins/research-log
```
...
```text
/plugin uninstall silotek-research-log@silotek
