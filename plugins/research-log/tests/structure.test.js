...
test('marketplace and package expose the research-log plugin in the silotek marketplace', () => {
  ...
  assert.equal(marketplace.name, 'silotek');
  assert.deepEqual(marketplace.plugins.map(p => p.name).sort(), ['research-log', 'serial-mcp']);
  const entry = marketplace.plugins.find(p => p.name === 'research-log');
  assert.ok(entry, 'marketplace must list the research-log plugin');
  assert.equal(entry.source.replace(/\\/g, '/'), './plugins/research-log');
  assert.equal(plugin.name, 'research-log');
  assert.equal(pkg.name, 'research-log');
});
