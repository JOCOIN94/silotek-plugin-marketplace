...
버전 이력: v1.0.0에서 marketplace ID를 `silotek`, 연구 로그 플러그인을 `research-log`, 시리얼 스킬을 `serial`, 서버 repo를 `serial-mcp-server`로 표준화(브레이킹). v0.3.0에서 `silotek-research-log` → `research-log` 이름 변경(브레이킹). v0.4.1은 소스/유형 선택과 병렬 다이어그램 생성을 유지하면서 다이어그램 스킬을 단일 Silotek 라이트 규칙 세트로 정리(비-브레이킹). v0.4.3은 DOCX `code` 블록 멀티라인 줄바꿈 복구 및 버전 동기화 흐름 간소화(비-브레이킹). v0.5.0은 연구 로그 파이프라인을 중앙 보관소 직행으로 단순화 — 작업 폴더 잔여물 제거, `save-draft`의 복사·경로 재작성 단계 폐지, `next-basename.js` 도입(비-브레이킹: 사용자 명령 표면과 산출물 경로 동일). v0.6.0은 작업 폴더 쓰기를 모든 흐름에서 차단 — `/diagram-create` 출력 경로를 중앙 `diagrams/<YYYY-MM-DD>/`로 이전, `next-diagram-path.js`의 작업 폴더 폴백 제거 및 `--standalone` 플래그 신설, `save-draft`/`resolve-yaml`/`build-docx`/`rasterize-svg`의 모든 쓰기 경로에 `assertInsideStorage`/`assertInsideSubdir` invariant 가드 추가(브레이킹: 독립 다이어그램 출력 경로 변경). v0.7.0은 다이어그램 스킬 디렉터리 이름을 `silotek-diagram-design` → `diagram-create`로 통일해 다른 짝(`research-log-*`)과 명령==스킬 이름 패턴을 맞춤(비-브레이킹: 명령·서브에이전트 동작 동일, 슬래시 자동완성 목록만 정리됨). v0.8.0은 명령 5개의 description을 한국어로 정리하고, `setup-check`에 GitHub raw `marketplace.json` 비교 기반 원격 버전 체크(`update` 항목)를 추가 — 네트워크 실패 시 `warn` 한 줄로 끝나며, `RESEARCH_LOG_SKIP_UPDATE_CHECK=1`로 비활성 가능, `RESEARCH_LOG_UPDATE_URL`로 출처 override 가능(비-브레이킹). serial-mcp v1.1.0은 `install-codex.ps1`에 `-SerialWrite`/`-SerialWriteConfirm` 파라미터를 추가해 Claude plugin.json의 env 패스스루 13종과 패리티를 맞춤(비-브레이킹). serial-mcp v1.2.0은 서버 0.3.0의 `SERIAL_CHAR_DELAY`(전송 문자 간 지연, 기본 10ms — STM32 폴링 수신 문자 유실 대응)를 plugin.json 패스스루와 `-SerialCharDelay` 파라미터로 노출(비-브레이킹). serial-mcp v1.2.1은 배포 기본 `SERIAL_CHAR_DELAY`를 100ms로 고정 — Claude plugin.json `${SERIAL_CHAR_DELAY:-100}`·Codex install-codex.ps1 기본 env 동일 적용, 근거는 SB-STM 실측(10ms 잔존 유실 → 100ms 유실 0, `atlas/sb-stm/exploration/2026-06-12-r1-chardelay100`). 서버 기본값 10ms는 유지(비-브레이킹). serial-mcp v1.3.0은 서버 코드를 미고정 `main`에서 태그 `@v1.1.0`로 pin — 팀 배포가 uv 캐시 무효화 없이 새 서버를 받도록 `plugin.json` args·`install-codex.ps1`·`verify-codex.ps1`를 일괄 갱신. 서버 v1.1.0은 `get_serial_status` 첫-open self-trigger race 가드(`_first_open_done` Event + 최대 1.5초 바운드 대기 + `opening` 필드 — 첫 호출의 거짓 `connected=false` 제거)와 웹 뷰어 기본포트 스마트 선택(데이터·연결 우선, localStorage 기억), `viewer_url` 안내 의무를 docstring·SPEC §5·SKILL §0에 일원화(비-브레이킹). serial-mcp v1.3.1은 서버 `@v1.1.1`로 pin — `list_serial_ports`가 owner 미획득(0개 모니터링)인데 USB 포트가 보이면 `get_serial_status`로 획득·자동 모니터링하라는 `hint` 필드·docstring을 추가하고, SKILL 함정에 'AI는 모니터 포트를 고르지 않는다 / list의 0개를 보고 사람에게 포트를 묻지 말 것 / 0개 지속 시 owner-busy로 보고 웹뷰 [해제] 안내'를 명시(비-브레이킹). 다른 세션이 점유 중이면 새 세션은 `list_serial_ports`만으로는 영영 0개라 헤매던 문제를 차단. serial-mcp v1.4.0은 서버를 `@v1.2.0`로 pin하고 승인 게이트를 등급-인지로 전환 — 서버 `SERIAL_WRITE_CONFIRM`을 3-state(`all`/`r3`/`off`)로 확장하고 `_confirm_write`가 R3(reflash/format/download/파일삭제/임의 JSON 주입 + boot-menu `D`)에만 elicitation 승인, 비-R3·`reset_board`(R2)는 통과. 배포 기본을 `r3`로 주입(Claude plugin.json `${SERIAL_WRITE_CONFIRM:-r3}`·Codex install-codex.ps1 기본 env), 서버 내부 기본 `all`은 유지해 다른 설치는 영향 없음. `plugin.json` args·`install-codex.ps1`·`verify-codex.ps1`를 `@v1.2.0`로 일괄 갱신, ops.md 위험 게이트에 서버 R3 강제·command-surface↔`_R3_COMMANDS` 동기화 invariant 명시. 근거: 승인 팝업은 서버측 elicitation(`ctx.elicit`)이라 CC PreToolUse hook으로 가로챌 수 없어 게이트는 서버에만 둔다(클라이언트 중립 — Claude·Codex 동일). 서버 v1.2.0은 `_R3_COMMANDS`·`_is_r3()`·`_parse_confirm_mode` 추가, 테스트 248개 통과(비-브레이킹). serial-mcp v1.5.0은 스킬 문서의 위험 게이트를 서버 `r3` 동작과 일치시킴 — `serial/SKILL.md`·`serial/references/ops.md`·`ssm`/`sb-esp` `command-surface.md`에서 R1·R2의 '사람 입회'/'멈춰 사람과 합의' 문구를 제거하고 R0–R2는 승인·입회·되묻기 없이 자동 실행+검증으로 명시(R3 실행 금지·변경 전후 snapshot/verify·interactive prompt 단계 진행·secret redaction·대상 모호 시 재관찰 안전장치는 유지). 근거: 서버는 이미 `SERIAL_WRITE_CONFIRM=r3`(plugin.json 배포 기본)로 R0–R2를 통과시키는데 문서만 사람 입회를 요구해 모순이었음 — 문서를 shipped 동작에 맞춘 것이다(비-브레이킹: 명령·risk 등급·서버 동작 불변, 문서 정책만 일치). serial-mcp v1.5.1은 서버를 `@v1.2.1`로 pin하고 보드 식별 규율을 docstring·ops·SPEC 전 표면에서 일치시킴 — `list_serial_ports` docstring·`ops.md`·`SPEC.md`의 '별칭이 진실이다'·'VID/PID로 칩 추론'을 '칩≠보드 / 별칭이 붙어 있으면 그것이 식별 기준 / 없으면 로그·사람으로 식별'로 통일하고, `get_serial_status` docstring의 viewer 포트 문구를 '재시작마다 바뀜'(사실 오류) → '8743 고정·SERIAL_WEB로만 변경'으로 교정, `clear_log_buffer` docstring에 '분석 중이면 비우기 전 snapshot·맥락 보존 시 선택 비우기' 가드를 추가. 서버 v1.2.1은 docstring·SPEC 문서만 수정(코드 로직 불변, 테스트 248개 통과). 근거: 스킬 미발동 시 모델이 따르는 실질 권위는 도구 docstring인데 그것이 스킬과 불일치하거나 사실 오류였음(예: viewer 포트) — 권위 표면 자체를 일치시킴(비-브레이킹: 서버 동작·명령·risk 등급 불변, 문서만 정합).
